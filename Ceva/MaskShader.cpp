...
CMaskShader::CMaskShader( )
{
	ZeroMemory( this, sizeof( CMaskShader ) );
}

bool CMaskShader::Initialize( ID3D11Device * device )
{
	ID3D10Blob *ShaderBlob, *ErrorBlob;
	HRESULT hr;
	hr = D3DX11CompileFromFile( L"MaskVertexShader.hlsl", NULL, NULL, "main", "vs_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL );
	if ( FAILED( hr ) )
	{
		if ( ShaderBlob )
	...
	hr = D3DX11CompileFromFile( L"MaskPixelShader.hlsl", NULL, NULL, "main", "ps_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL );
	if ( FAILED( hr ) )
	{
		if ( ShaderBlob )
...
void CMaskShader::Render( ID3D11DeviceContext * context, UINT IndexDrawAmount, ID3D11ShaderResourceView * texture,
...
void CMaskShader::OutputShaderError( ID3D10Blob * Error )
{
