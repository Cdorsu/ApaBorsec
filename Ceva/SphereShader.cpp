...
bool SphereShader::Initialize( ID3D11Device * device )
{
	ID3D10Blob *ShaderBlob = nullptr, *ErrorBlob = nullptr;
	HRESULT hr = S_OK;
	hr = D3DX11CompileFromFile( L"SphereVertexShader.hlsl", NULL, NULL, "main", "vs_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL );
	if ( FAILED( hr ) )
	{
		if ( ShaderBlob )
	...
	hr = D3DX11CompileFromFile( L"SpherePixelShader.hlsl", NULL, NULL, "main", "ps_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL );
	if ( FAILED( hr ) )
	{
		if ( ShaderBlob )
	...
	if ( FAILED( hr ) )
		return false;
	hr = D3DX11CompileFromFile( L"SphereGeometryShader.hlsl", NULL, NULL, "main", "gs_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL );
	if ( FAILED( hr ) )
	{
		if ( ShaderBlob )
			OutputShaderError( ErrorBlob );
...
void SphereShader::Render(
	ID3D11DeviceContext * context, UINT IndexDrawAmount,
...
)
{
	...
	( ( SConstantBuffer* ) MappedResource.pData )->EyePos = CamPos;
...
void SphereShader::OutputShaderError( ID3D10Blob * Error )
{
