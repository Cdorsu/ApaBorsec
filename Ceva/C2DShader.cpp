...
bool C2DShader::Initialize( ID3D11Device * device )
{
	...
	if (FAILED( hr ))
		return false;
	buffDesc.ByteWidth = sizeof( SColorBuffer );
	hr = device->CreateBuffer( &buffDesc, NULL, &ColorBuffer );
	if (FAILED( hr ))
		return false;
...
void C2DShader::Render( ID3D11DeviceContext * context, UINT IndexDrawAmount, ID3D11ShaderResourceView * ShaderResource,
	DirectX::XMMATRIX& World, DirectX::XMMATRIX& View, DirectX::XMMATRIX& Projection, common::Color Color )
{
	...
	hr = context->Map( ColorBuffer, 0, D3D11_MAP::D3D11_MAP_WRITE_DISCARD, 0, &MappedResource );
	if (FAILED( hr ))
		return;
	((SColorBuffer*)MappedResource.pData)->Color = Color;
