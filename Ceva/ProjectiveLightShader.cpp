...
CProjectiveLightShader::CProjectiveLightShader( )
{
	ZeroMemory( this, sizeof( CProjectiveLightShader ) );
}

bool CProjectiveLightShader::Initialize( ID3D11Device * device )
{
	ID3D10Blob *ShaderBlob = nullptr, *ErrorBlob = nullptr;
	HRESULT hr = S_OK;
	hr = D3DX11CompileFromFile( L"ProjectiveLightingVS.hlsl", NULL, NULL, "main", "vs_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL );
	if ( FAILED( hr ) )
	{
		if ( ErrorBlob )
	...
	hr = D3DX11CompileFromFile( L"ProjectiveLightingPS.hlsl", NULL, NULL, "main", "ps_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL );
	if ( FAILED( hr ) )
	{
		if ( ErrorBlob )
			OutputShaderError( ErrorBlob );
...
void CProjectiveLightShader::Render( ID3D11DeviceContext * context, UINT IndexDrawAmount,
...
void CProjectiveLightShader::OutputShaderError( ID3D10Blob * Error )
{
...
void CProjectiveLightShader::Shutdown( )
{
