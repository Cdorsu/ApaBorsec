...
{
	ZeroMemory(this, sizeof(CExplosionShader));
}

bool CExplosionShader::Initialize(ID3D11Device * device)
{
	ID3D10Blob *ShaderBlob = nullptr, *ErrorBlob = nullptr;
	HRESULT hr = S_OK;
	hr = D3DX11CompileFromFile(L"ExplosionVertexShader.hlsl", NULL, NULL, "main", "vs_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL);
	if (FAILED(hr))
	{
		if (ShaderBlob)
	...
	hr = D3DX11CompileFromFile(L"ExplosionPixelShader.hlsl", NULL, NULL, "main", "ps_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL);
	if (FAILED(hr))
	{
		if (ShaderBlob)
	...
	if (FAILED(hr))
		return false;
	hr = D3DX11CompileFromFile(L"ExplosionGeometryShader.hlsl", NULL, NULL, "main", "gs_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL);
	if (FAILED(hr))
	{
		if (ShaderBlob)
			OutputShaderError(ErrorBlob);
	...
	hr = device->CreateBuffer(&buffDesc, NULL, &TimeBuffer);
	if (FAILED(hr))
		return false;
...
void CExplosionShader::Render(ID3D11DeviceContext * context, UINT IndexDrawAmount,
	...
	float t)
{
	...
	context->Map(TimeBuffer, 0, D3D11_MAP::D3D11_MAP_WRITE_DISCARD, 0, &MappedResource);
	((SCamPos*)MappedResource.pData)->T = t;
	context->Unmap(TimeBuffer, 0);
	context->GSSetConstantBuffers(1, 1, &TimeBuffer);
...
void CExplosionShader::OutputShaderError(ID3D10Blob * Error)
{
...
void CExplosionShader::Shutdown()
{
