...
bool CHorizontalBlurShader::Initialize( ID3D11Device * device )
{
	...
	HRESULT hr;
	hr = D3DX11CompileFromFile( L"HorizontalBlurVertexShader.hlsl", NULL, NULL, "main", "vs_4_0", NULL, NULL, NULL, &ShaderBlob, &ErrorBlob, NULL );
	if ( FAILED( hr ) )
	{
		if ( ShaderBlob )
...
void CHorizontalBlurShader::Render( ID3D11DeviceContext * context, UINT IndexDrawAmount, ID3D11ShaderResourceView * ShaderResource,
	DirectX::XMMATRIX& World, DirectX::XMMATRIX& View, DirectX::XMMATRIX& Projection, float windowWidth )
{
	...
	if ( FAILED( hr ) )
		return;
	( ( SWindowInfo* ) MappedResource.pData )->windowWidth = windowWidth;
...
void CHorizontalBlurShader::OutputShaderError( ID3D10Blob * Error )
{
